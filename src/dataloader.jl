...
function load_data(path::String="input.txt"; shuffle::Bool=true, rng=Random.GLOBAL_RNG)::Vector{String}
    if !isfile(path)
        #		@info "Downloading names dataset to $path ..."
        Downloads.download(NAMES_URL, path)
    end
    docs = [strip(l) for l in eachline(path) if !isempty(strip(l))]
    #	Filter auf a-z?
    #	docs = filter(doc -> all(c -> 'a' <= c <= 'z', doc), docs)  # nur a-z
    shuffle && Random.shuffle!(rng, docs)
